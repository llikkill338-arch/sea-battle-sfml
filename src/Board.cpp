// ============================================================================
// Board.cpp - Пиратское игровое поле (v4.0 - Victory fix + Aura)
// ============================================================================
...
Board::Board(float ox, float oy, bool enemy) : offsetX(ox), offsetY(oy), isEnemy(enemy) {
    clear();
}
...
// FIXED: Now properly calls hitPart() to track ship damage
bool Board::shoot(int r, int c) {
    ...
    if (s == CellState::Ship) {
        ...
        // CRITICAL FIX: Mark the ship part as damaged
        for (auto& ship : ships) {
            if (ship->contains(r, c)) {
                ship->hitPart(r, c);  // <-- THIS WAS MISSING!

                // Check if ship is now sunk -> mark aura
                if (ship->isSunk()) {
                    markShipAura(ship->getRow(), ship->getCol(),
                                 ship->getSize(), ship->isHorizontal());
                }
                break;
            }
        }
    ...
    for (int r = 0; r < BOARD_SIZE; r++) {
        for (int c = 0; c < BOARD_SIZE; c++) {
            ...
            if (ghostSize > 0 && cursorR >= 0 && cursorC >= 0) {
                if (ghostDir) isGhost = (r == cursorR && c >= cursorC && c < cursorC + ghostSize);
                else          isGhost = (c == cursorC && r >= cursorR && r < cursorR + ghostSize);
            }
...
void Board::drawGrid(sf::RenderWindow& window, sf::Font& font) {
    sf::RectangleShape border(sf::Vector2f(BOARD_PIXELS + 40, BOARD_PIXELS + 40));
    ...
    for (int c = 0; c < BOARD_SIZE; c++) {
        ...
        float tx = offsetX + 30 + c * (CELL_SIZE + CELL_MARGIN) + CELL_SIZE / 2 - 5;
        text.setPosition(tx, offsetY + 10);
        window.draw(text);
    }
    for (int r = 0; r < BOARD_SIZE; r++) {
        char letter = 'A' + r;
        ...
        text.setFillColor(TEXT_GOLD);
        text.setPosition(offsetX + 12,
            offsetY + 35 + r * (CELL_SIZE + CELL_MARGIN) + CELL_SIZE / 2 - 8);
        window.draw(text);
    }
}
